...
차별화 포인트 (반드시 반영):
0) 종목 추출 능력: 사용자 입력(텍스트 또는 이미지)에서 종목명(티커)을 **최대한 모두** 추출해낸다. 종목이 10개든 20개든 개수에 상관없이 사용자가 언급한 모든 종목을 portfolio_tickers 배열에 담아야 한다. 만약 사용자가 "삼성전자 10주, 하이닉스 5주, 애플 2주, 테슬라 1주, 엔비디아 3주..." 처럼 길게 입력했다면 반드시 그 모든 종목을 추출한다. 수량이나 현재가 등의 불필요한 정보는 무시하되, 종목 이름은 하나도 빠짐없이 추출한다.
...
- news_feed 배열에 3~6개 뉴스 아이템. 각 아이템:
  ...
  - sector: "반도체" | "자동차" | "에너지" | "금융" | "빅테크" | "소비재" 등 핵심 산업 섹터 명시
...
응답은 반드시 아래 JSON만 출력:
{
  ...
  "portfolio_tickers": ["..."],
  "portfolio_links": [
    { "source": "거시_축_키(예:currency)", "target": "종목명", "reason": "연결 이유 요약", "strength": "high|medium|low" }
  ],
  "risk_matrix": {
...
export function buildUserPrompt({
  ...
  if (skipPortfolio) {
    ...
    portfolioLine = `사용자 제공 포트폴리오 텍스트 (자연어/비정형 포함): "${textTickers.trim()}"\n위 텍스트에서 종목명/티커만 정교하게 추출해서 반영해 주세요.`
